...
def M15():#me retorna el tipo de vela ultima cerrada en M15 ('alcista','bajista')
    global simbolo
...
def M1():#retorna el ultimo tipo de vela cerrada en M1
    global simbolo
...
def M1ante():#retorna el tipo de vela de la vela anterior a la ultima cerrada
    global simbolo
    data=pd.DataFrame((mt5.copy_rates_from_pos(simbolo,mt5.TIMEFRAME_M1,0,10)))
    data=heikinAshi(data)#Dataframe 
    data=numpyAlist(data)#list[[open,high,low,close]]
...
def cerrarOrden(ticket,lote,tipo):
    global simbolo
    precio=mt5.symbol_info_tick(simbolo).ask
    deviation = 0
    if tipo == 'compra':
...
def cuandoCerrar():
    ...
    while 1:
        ...
        if len(operaciones)>0:
            tikect=operaciones[0][0]
            tipo=operaciones[0][5]
            lote=operaciones[0][9]
            if tipo==0:
                tipo='compra'
            else:
                tipo='venta'
...
def inicio(riesgo):
    ...
    while 1:
        ...
        if spread==100 or spread==10:
            ...
            primero=0
            rompimiento=0
            num_operaciones=mt5.positions_total()
            if num_operaciones==0:
                ...
                if m15=='alcista':
                    if m1=='alcista':
                        ...
                        if rompimiento<primero and ultimoimpulso==rompimiento and M1ante()=='bajista':
                            print("compra")# 0 si es compra y 1 si es venta
                            precio=mt5.symbol_info_tick(simbolo).ask
                ...
                if m15=='bajista':
                    if m1=='bajista':
                        ...
                        if rompimiento>primero and ultimoimpulso==rompimiento and M1ante()=='alcista':
                            print("venta")#1 si es venta y 0 si es compra 
                            precio=mt5.symbol_info_tick(simbolo).ask
                            ...
                            if lote<0.1:#lotaje minimo permitido es 0.1
                                lote=0.1
                            lote=float(lote)
                            print(ordenar(1,lote,tp,sl))
        time.sleep(0.2)
        
#--------------------------------------------------------------------------------------------------------------
riesgo=3  #   1 = 1% de de la cuenta
simbolo='USTEC'
#--------------------------------------------------------------------------------------------------------------
cuenta=7140257
contraseña="p6ifHZXG"
...
ruta="C:/Users/adjua/Desktop/Terminales/MIO/metaC/terminal64.exe"
autorizar=mt5.initialize(ruta,login=cuenta,Password=contraseña,server=servidor)
if autorizar:
    print("CUENTA HEIKEN ASHI: "+str(cuenta))
else:
